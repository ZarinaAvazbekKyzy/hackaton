...
//!create function that shows users
function readUsers() {
  fetch(`${API}?q=${searchValue}&_page=${currentPage}&_limit=6`)
    ...
    <table>
      ...
      <th><button class="btn btn-success btnDelete" id="${data[i].id}">Delete</button></th>
...
btnEditSave.addEventListener("click", () => {
  ...
  editUser(editedUser, btnEditSave.id);
});
function editUser(editObj, id) {
  fetch(`${API}/${id}`, {
    method: "PATCH",
